...
def test_scrub_stripe_and_url_creds():
    # These are FABRICATED provider-shaped fixtures, not real keys. Assemble the
    # anchored prefixes from fragments (as test_secrets.py already does) so no
    # complete Stripe-shaped token sits contiguous in the source — otherwise
    # secret scanners flag the git blob (GitHub alert #1, 2026-07-19). The
    # runtime value is identical, so the scrubber regex is still exercised.
    stripe_key = "sk_" + "live_abcDEF12345678901234"
    out = scrub_secrets(f"use {stripe_key} for prod")
    assert stripe_key not in out and REDACTED in out
    webhook_secret = "wh" + "sec_AbCdEf123456789012345678"
    out = scrub_secrets(f"{webhook_secret} is the webhook secret")
    assert webhook_secret not in out and REDACTED in out
