...
def _register(mcp: FastMCP, app, read_only: bool):
    @mcp.tool(name="search", annotations=READ_ONLY,
              description=("Search the Mainframe (hybrid vector+keyword, reranked). Use SPECIFIC technical "
                           "terms; trust rerank_score (higher = better). ranked_by names the model that "
                           "ordered these results, or is null when nothing did (empty results, or the "
                           "reranker is disabled). response_format: concise (default) "
